...
public sealed class ESignMobileSignedDocumentsController : ControllerBase
{
    ...
    private static bool TryGetNombreDocumentoOriginal(string fileName, out string nombreOriginal)
    {
        ...
        nombre = nombre.Replace("-firmado", "_firmado", StringComparison.OrdinalIgnoreCase);
        var legado = System.Text.RegularExpressions.Regex.Match(nombre, "^firmar-\\d+-(.+)-\\d+_firmado$", System.Text.RegularExpressions.RegexOptions.IgnoreCase);
        if (legado.Success)
        {
            nombreOriginal = legado.Groups[1].Value.Trim();
        ...
        try { nombreOriginal = Uri.UnescapeDataString(nombreOriginal); }
        catch (UriFormatException) { }
        nombreOriginal = nombreOriginal.Replace('_', ' ').Trim();
        return !string.IsNullOrWhiteSpace(nombreOriginal);
    }
}
